...
启动后浏览器访问 `http://127.0.0.1:8080`（若 8080 被占用，可在 `config.toml` 将 `server.port` 设为 `0` 自动分配空闲端口，或设为其他端口）。

### 端口与冲突处理

- 桌面客户端 `dodogo-desktop.exe` **不依赖固定端口**：启动时自动探测一个空闲端口拉起服务（避开 8080 等易冲突端口）；若检测到 8080 已有 DoDoGo 实例在运行则直接复用。
- 服务端 `server.port = 0` 时由系统自动分配空闲端口，实际端口会写入 `data/server.port` 文件（供客户端/脚本发现）。
...
| 关键项 | 说明 |
| --- | --- |
| `server.host` / `server.port` | 监听地址（默认 127.0.0.1:8080；`port = 0` 自动分配空闲端口） |
