...
(() => {
  ...
  function addPasswordToggles(root) {
    root.querySelectorAll('input[type="password"]').forEach((input) => {
      ...
      const wrap2 = el("div", { class: "pwd-wrap" });
      input.parentNode?.insertBefore(wrap2, input);
  ...
  function hexToRgba(hex, alpha2) {
    ...
    if (!m) return `rgba(59,130,246,${alpha2})`;
  ...
  function formField(label, control, hint = "") {
    ...
    wrap2.append(el("label", { text: label }));
    wrap2.append(control);
    if (hint) wrap2.append(el("div", { class: "field-hint muted", text: hint }));
    return wrap2;
  }
  ...
  var DecorationGroup = class _DecorationGroup {
    ...
    static from(members2) {
      switch (members2.length) {
        case 0:
          return empty;
        case 1:
          return members2[0];
        default:
          return new _DecorationGroup(members2.every((m) => m instanceof DecorationSet) ? members2 : members2.reduce((r, m) => r.concat(m instanceof DecorationSet ? m : m.members), []));
      }
    }
    forEachSet(f) {
      for (let i = 0; i < this.members.length; i++)
...
3. "-" cannot repeat`);
  ...
  var CellSelection = class CellSelection2 extends Selection {
    ...
    forEachCell(f) {
      ...
      for (let i = 0; i < cells.length; i++) f(table2.nodeAt(cells[i]), tableStart + cells[i]);
    }
    isColSelection() {
  ...
  function addColumn(tr2, { map: map2, tableStart, table: table2 }, col) {
    ...
    for (let row = 0; row < map2.height; row++) {
      ...
      if (col > 0 && col < map2.width && map2.map[index - 1] == map2.map[index]) {
        ...
        const type = refColumn == null ? tableNodeTypes(table2.type.schema).cell : table2.nodeAt(map2.map[index + refColumn]).type;
        const pos = map2.positionAt(row, col, table2);
  ...
  function removeColumn(tr2, { map: map2, table: table2, tableStart }, col) {
  ...
  function rowIsHeader(map2, table2, row) {
    var _table$nodeAt;
    const headerCell = tableNodeTypes(table2.type.schema).header_cell;
    for (let col = 0; col < map2.width; col++) if (((_table$nodeAt = table2.nodeAt(map2.map[col + row * map2.width])) === null || _table$nodeAt === void 0 ? void 0 : _table$nodeAt.type) != headerCell) return false;
    return true;
  }
  function addRow(tr2, { map: map2, tableStart, table: table2 }, row) {
    let rowPos = tableStart;
    for (let i = 0; i < row; i++) rowPos += table2.child(i).nodeSize;
    const cells = [];
    let refRow = row > 0 ? -1 : 0;
    if (rowIsHeader(map2, table2, row + refRow)) refRow = row == 0 || row == map2.height ? null : 0;
    for (let col = 0, index = map2.width * row; col < map2.width; col++, index++) if (row > 0 && row < map2.height && map2.map[index] == map2.map[index - map2.width]) {
      ...
      var _table$nodeAt2;
      const type = refRow == null ? tableNodeTypes(table2.type.schema).cell : (_table$nodeAt2 = table2.nodeAt(map2.map[index + refRow * map2.width])) === null || _table$nodeAt2 === void 0 ? void 0 : _table$nodeAt2.type;
  ...
  function removeRow(tr2, { map: map2, table: table2, tableStart }, row) {
    let rowPos = 0;
    for (let i = 0; i < row; i++) rowPos += table2.child(i).nodeSize;
    ...
    for (let col = 0, index = row * map2.width; col < map2.width; col++, index++) {
      ...
      if (row > 0 && pos == map2.map[index - map2.width]) {
        ...
        const newPos = map2.positionAt(row + 1, col, table2);
  ...
  function isolateHorizontal(tr2, map2, table2, start, left, right, top, mapFrom) {
    ...
    for (let col = left; col < right; col++) {
      ...
      if (map2.map[index - map2.width] == pos) {
        ...
        tr2.insert(tr2.mapping.slice(mapFrom).map(map2.positionAt(top, cellLeft, table2)), cell.type.createAndFill({
          ...cell.attrs,
  ...
  function insertCells(state, dispatch, tableStart, rect, cells) {
    ...
    if (growTable(tr2, map2, table2, tableStart, right, bottom, mapFrom)) recomp();
    if (isolateHorizontal(tr2, map2, table2, tableStart, left, right, top, mapFrom)) recomp();
    if (isolateHorizontal(tr2, map2, table2, tableStart, left, right, bottom, mapFrom)) recomp();
    if (isolateVertical(tr2, map2, table2, tableStart, top, bottom, left, mapFrom)) recomp();
    if (isolateVertical(tr2, map2, table2, tableStart, top, bottom, right, mapFrom)) recomp();
    for (let row = top; row < bottom; row++) {
      const from2 = map2.positionAt(row, left, table2), to = map2.positionAt(row, right, table2);
  ...
  function inline(node) {
    ...
    switch (el2.tagName) {
      ...
      case "BR":
        return "  \n";
      case "INPUT": {
        if ((el2.getAttribute("type") || "").toLowerCase() !== "checkbox") return "";
...
`;
  ...
  function list(el2, ordered) {
    ...
    for (const child of Array.from(el2.children)) {
      ...
      const first2 = (isTask ? checked ? "[x] " : "[ ] " : "") + lines[0];
      out += marker + first2 + "\n";
  ...
  // ts/editor.ts
  function liHasCheckbox(li) {
    return Array.from(li.childNodes).some(
      (n) => n.nodeType === Node.ELEMENT_NODE && n.tagName === "INPUT" && (n.getAttribute("type") || "").toLowerCase() === "checkbox"
    );
  }
  function convertTaskItem(li) {
    const input = Array.from(li.childNodes).find(
      (n) => n.nodeType === Node.ELEMENT_NODE && n.tagName === "INPUT" && (n.getAttribute("type") || "").toLowerCase() === "checkbox"
    );
    const checked = input ? input.hasAttribute("checked") : false;
    if (input) input.remove();
    const doc3 = li.ownerDocument;
    li.setAttribute("data-type", "taskItem");
    li.setAttribute("data-checked", checked ? "true" : "false");
    const label = doc3.createElement("label");
    const cb = doc3.createElement("input");
    cb.setAttribute("type", "checkbox");
    if (checked) cb.setAttribute("checked", "checked");
    label.append(cb, doc3.createElement("span"));
    const div = doc3.createElement("div");
    const p = doc3.createElement("p");
    const blockChildren = [];
    for (const n of Array.from(li.childNodes)) {
      if (n.nodeType === Node.ELEMENT_NODE && ["UL", "OL", "BLOCKQUOTE", "PRE"].includes(n.tagName)) {
  ...
  function convertTaskLists(doc3) {
    const uls = Array.from(doc3.querySelectorAll("ul")).filter(
      (ul) => Array.from(ul.children).some(
        (c) => c.tagName === "LI" && liHasCheckbox(c)
      )
    );
    for (const ul of uls) {
      ...
      for (const g of groups) {
        const newUl = doc3.createElement("ul");
  ...
  function buildDescription(d) {
    ...
    function showEditor() {
      ...
      if (!initialized) {
        initialized = true;
        loading = true;
        void wys.setMarkdown(d.description).then(() => wys.focus()).catch(() => wys.focus()).finally(() => {
          loading = false;
        });
      } else {
        wys.focus();
  ...
  function buildChecklists(d) {
    ...
    for (const cl of d.checklists) {
      ...
      for (const item of cl.items) {
        ...
        const del2 = el("button", { class: "btn-icon", type: "button", text: "\u2715", title: "\u5220\u9664\u6761\u76EE" });
  ...
  function buildAddChecklistItem(clId) {
    const wrap2 = el("div", { class: "add-inline" });
  ...
  function buildComments(d) {
    ...
    const ta2 = el("textarea", { class: "input", rows: "3", placeholder: "\u8F93\u5165\u8BC4\u8BBA\uFF0C\u652F\u6301 Markdown\uFF1B\u53EF\u76F4\u63A5\u7C98\u8D34\u56FE\u7247\u4E0A\u4F20" });
    ta2.addEventListener("paste", async (e) => {
    ...
    editor.append(ta2, hint, actions);
    sec.append(editor);
    return sec;
  }
  ...
  function buildAttachments(d) {
    ...
    for (const a of d.attachments) {
      ...
      const del2 = el("button", { class: "btn-icon", type: "button", text: "\u2715", title: "\u5220\u9664\u9644\u4EF6" });
      del2.addEventListener("click", async () => {
      ...
      row.append(link, el("span", { class: "muted", text: fmtSize(a.fileSize) }), el("span", { class: "muted", text: a.uploaderName + " \xB7 " + timeAgo(a.createdAt) }), del2);
      list2.append(row);
    }
    sec.append(list2);
  ...
  function buildActivity(d) {
    ...
    for (const a of d.activities) {
      ...
      row.append(avatar({ id: a.userId ?? void 0, avatarPath: a.avatarPath, displayName: a.displayName, username: a.username }, "xs"));
  ...
  function buildAddCard(colId) {
    const wrap2 = el("div", { class: "add-card" });
    ...
    wrap2.append(btn, form);
    return wrap2;
  }
  function buildAddColumn() {
    const wrap2 = el("div", { class: "add-col-wrap" });
  ...
  function statusLabel(kind, status) {
    const map2 = kind === "milestones" ? MILESTONE_STATUS : RELEASE_STATUS;
    return map2.find(([v]) => v === status)?.[1] || status;
  }
  ...
  function render2(listEl, members2, reload) {
    ...
    for (const m of members2) {
      ...
      tr2.append(roleTd);
      tr2.append(el("td", { class: "muted", text: fmtDate(m.joinedAt) }));
      const opTd = el("td");
      if (m.role !== "owner") {
        const del2 = el("button", { class: "btn btn-ghost btn-sm btn-danger-text", type: "button", text: "\u79FB\u9664" });
        del2.addEventListener("click", async () => {
  ...
  function openAddModal(projectKey3, existingIds, reload) {
    ...
    const wrap2 = el("div", { class: "autocomplete" });
    const dropdown = el("div", { class: "autocomplete-list", hidden: "true" });
    wrap2.append(identityInput, dropdown);
    body.append(formField("\u6210\u5458\uFF08\u6A21\u7CCA\u641C\u7D22\uFF09", wrap2));
    ...
    document.addEventListener("pointerdown", (e) => {
      if (!wrap2.contains(e.target)) dropdown.hidden = true;
    });
  ...
  function projectIcon(p, size = "md") {
    ...
    const wrap2 = el("span", { class: cls, text });
    wrap2.style.background = p.iconColor || "#3B82F6";
    if (p.iconPath) {
  ...
  function initSearch() {
    ...
    input.addEventListener("input", () => run3(input.value.trim()));
    input.addEventListener("keydown", (e) => {
      if (e.key === "Enter") run3(input.value.trim());
    });
  ...
  async function adminOverview(content) {
    ...
    const loadBackups = async () => {
      try {
        ...
        if (!list2.length) {
          const tr2 = el("tr");
          tr2.append(el("td", { colspan: "4", class: "muted", text: "\u6682\u65E0\u5907\u4EFD" }));
          tbody.append(tr2);
        }
        for (const b of list2) {
          const tr2 = el("tr");
          tr2.append(el("td", { text: b.name }));
          tr2.append(el("td", { class: "muted", text: fmtSize(b.size) }));
          tr2.append(el("td", { class: "muted", text: b.modifiedAt ? fmtDateTime(b.modifiedAt) : "" }));
          const op = el("td");
          const del2 = el("button", { class: "btn btn-ghost btn-sm btn-danger-text", type: "button", text: "\u5220\u9664" });
          del2.addEventListener("click", async () => {
    ...
    const render5 = async () => {
      ...
      try {
        ...
        for (const u of d.items) {
          ...
          tr2.append(statusTd);
          tr2.append(el("td", { class: "muted", text: u.createdAt ? fmtDateTime(u.createdAt) : "" }));
          ...
          const reset2 = el("button", { class: "btn btn-ghost btn-sm", type: "button", text: "\u91CD\u7F6E\u5BC6\u7801" });
          ...
          opTd.append(toggle, reset2, del2);
          tr2.append(opTd);
          tbody.append(tr2);
        }
        table2.append(tbody);
  ...
  async function adminSettings(content) {
    ...
    let map2;
    try {
      map2 = await api("/admin/settings");
    } catch (e) {
      content.innerHTML = `<div class="empty">${esc(errMsg(e))}</div>`;
      return;
    }
    const defaults2 = { allow_registration: 1, wip_mode: "warn" };
    for (const [k, v] of Object.entries(defaults2)) {
    ...
    regCheck.checked = String(map2.allow_registration) === "1" || map2.allow_registration === true;
    ...
    for (const [v, t] of [["warn", "\u4EC5\u63D0\u793A\uFF08\u5141\u8BB8\u8D85\u9650\uFF09"], ["block", "\u963B\u6B62\u8D85\u9650\u79FB\u52A8"]]) {
      const opt = el("option", { value: v, text: t });
      if (String(map2.wip_mode) === v) opt.selected = true;
      wipSel.append(opt);
    }
    ...
    for (const [k, v] of Object.entries(map2)) addRow2(k, v);
  ...
  async function adminAudit(content) {
    ...
    const render5 = async () => {
      ...
      try {
        ...
        if (!items.length) {
          const tr2 = el("tr");
          tr2.append(el("td", { colspan: "5", class: "muted", text: "\u6682\u65E0\u65E5\u5FD7" }));
          tbody.append(tr2);
        }
        for (const it of items) {
          const tr2 = el("tr");
          tr2.append(el("td", { class: "muted", text: fmtDateTime(it.createdAt) }));
          tr2.append(el("td", { text: it.username || "\u7CFB\u7EDF" }));
          tr2.append(el("td", { text: it.action }));
          tr2.append(el("td", { class: "muted", text: `${it.targetType || "-"}${it.targetId ? " #" + it.targetId : ""}` }));
          tr2.append(el("td", { class: "muted", text: it.ip || "-" }));
          tbody.append(tr2);
        }
        table2.append(tbody);
        content.append(table2);
  ...
  function initUserProfileLinks() {
    document.addEventListener("click", (e) => {
      const t = e.target;
      const link = t.closest("a.user-link");
      if (!link) return;
